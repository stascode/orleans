...
namespace UnitTestGrains
{
    internal class SimpleSelfManagedGrain : Grain, ISimpleSelfManagedGrain
    {
        ...
        public Task<ISimpleSelfManagedGrain> GetGrainReference()
        {
            return Task.FromResult(this.AsReference<ISimpleSelfManagedGrain>());
        }
        ...
        public Task<IGrain[]> GetMultipleGrainInterfaces_Array()
        {
            ...
            {
                grains[i] = GrainFactory.GetGrain<ISimpleSelfManagedGrain>(i);
            }
        ...
        //{
        //    IAddressable sessionGrain = GrainFactory.GetGrain<ISimpleSelfManagedGrain>(id);
    ...
    public class ProxyGrain : Grain, IProxyGrain
    {
        ...
        {
            proxy = GrainFactory.GetGrain<ISimpleSelfManagedGrain>(key);
            return TaskDone.Done;
        }
    ...
    internal class StressSelfManagedGrain : Grain, IStressSelfManagedGrain
    {
        ...
        public Task<IStressSelfManagedGrain> GetGrainReference()
        {
            return Task.FromResult(this.AsReference<IStressSelfManagedGrain>());
        }
        ...
        public Task PingOthers(long[] others)
        {
            ...
            {
                IStressSelfManagedGrain g1 = GrainFactory.GetGrain<IStressSelfManagedGrain>(key);
    ...
    internal class ReentrantStressSelfManagedGrain : Grain, IReentrantStressSelfManagedGrain
    {
        ...
        public Task PingMutableArray(byte[] data, long nextGrain, bool nextGrainIsRemote)
        {
            if (nextGrain > 0)
            {
                ...
                {
                    return GrainFactory.GetGrain<IReentrantStressSelfManagedGrain>(nextGrain).PingMutableArray(data, -1, false);
                }
                else
                {
                    return GrainFactory.GetGrain<IReentrantLocalStressSelfManagedGrain>(nextGrain).PingMutableArray(data, -1, false);
                }
            }
            return TaskDone.Done;
        }
        ...
        public Task PingImmutableArray(Immutable<byte[]> data, long nextGrain, bool nextGrainIsRemote)
        {
            if (nextGrain > 0)
            {
                if (nextGrainIsRemote)
                {
                    return GrainFactory.GetGrain<IReentrantStressSelfManagedGrain>(nextGrain).PingImmutableArray(data, -1, false);
                }
                else
                {
                    return GrainFactory.GetGrain<IReentrantLocalStressSelfManagedGrain>(nextGrain).PingImmutableArray(data, -1, false);
                }
            }
            return TaskDone.Done;
        }
        ...
        public Task PingMutableDictionary(Dictionary<int, string> data, long nextGrain, bool nextGrainIsRemote)
        {
            if (nextGrain > 0)
            {
                if (nextGrainIsRemote)
                {
                    return GrainFactory.GetGrain<IReentrantStressSelfManagedGrain>(nextGrain).PingMutableDictionary(data, -1, false);
                }
                else
                {
                    return GrainFactory.GetGrain<IReentrantLocalStressSelfManagedGrain>(nextGrain).PingMutableDictionary(data, -1, false);
                }
            }
            return TaskDone.Done;
        }
        ...
        public Task PingImmutableDictionary(Immutable<Dictionary<int, string>> data, long nextGrain, bool nextGrainIsRemote)
        {
            if (nextGrain > 0)
            {
                if (nextGrainIsRemote)
                {
                    return GrainFactory.GetGrain<IReentrantStressSelfManagedGrain>(nextGrain).PingImmutableDictionary(data, -1, false);
                }
                else
                {
                    return GrainFactory.GetGrain<IReentrantLocalStressSelfManagedGrain>(nextGrain).PingImmutableDictionary(data, -1, false);
                }
            }
            return TaskDone.Done;
        }
