...
namespace UnitTests.ActivationsLifeCycleTests
{
    ...
    public class DeactivateOnIdleTests
    {
        ...
        public void Reset()
        {
            ...
            {
                Console.WriteLine("TestCleanup failed with {0}: {1}", ex, ex.StackTrace);
            }
        }
        ...
        {
            UnitTestBase.Initialize(TestOptions.Copy());
            var a = GrainClient.GrainFactory.GetGrain<ICollectionTestGrain>(1, "UnitTestGrains.CollectionTestGrain");
            await a.IncrCounter();
        ...
        {
            UnitTestBase.Initialize(TestOptions.Copy());
            var a = GrainClient.GrainFactory.GetGrain<ICollectionTestGrain>(1, "UnitTestGrains.ReentrantCollectionTestGrain");
            await a.IncrCounter();
        ...
        private async Task DeactivateOnIdle_NonExistentActivation_Runner(int forwardCount)
        {
            ...
            unitTest.logger.Info("About to make a 1st GetAge() call.");
            ...
            try
            {
                unitTest.logger.Info("About to make a 2nd GetAge() call.");
