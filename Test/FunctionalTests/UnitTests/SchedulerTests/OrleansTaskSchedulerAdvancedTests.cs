...
namespace UnitTests.SchedulerTests
{
    ...
    public class OrleansTaskSchedulerAdvancedTests : MarshalByRefObject
    {
        ...
        public void Sched_AC_Test()
        {
            ...
            orleansTaskScheduler = UnitTestBase.InitializeSchedulerForTesting(context);
        ...
        {
            orleansTaskScheduler = UnitTestBase.InitializeSchedulerForTesting(new UnitTestSchedulingContext());
        ...
        public void Sched_AC_Turn_Execution_Order()
        {
            ...
            orleansTaskScheduler.QueueWorkItem(new ClosureWorkItem(() =>
            {
                ...
                Task task4 = task3.ContinueWith((_) => { SubProcess1(14); resultHandle1.Done = true; });
                task4.Ignore();

                Task task21 = TaskDone.Done.ContinueWith((_) => SubProcess2(21));
                Task task22 = task21.ContinueWith((_) => { SubProcess2(22); resultHandle2.Done = true; });
                task22.Ignore();
        ...
        public void Sched_Task_Turn_Execution_Order()
        {
            ...
            UnitTestSchedulingContext context = new UnitTestSchedulingContext();
            OrleansTaskScheduler masterScheduler = orleansTaskScheduler = UnitTestBase.InitializeSchedulerForTesting(context);
        ...
        {
            UnitTestSchedulingContext context = new UnitTestSchedulingContext();
            OrleansTaskScheduler orleansTaskScheduler = orleansTaskScheduler = UnitTestBase.InitializeSchedulerForTesting(context);
            ...
            orleansTaskScheduler.QueueWorkItem(new ClosureWorkItem(() =>
                {
                    ...
                    Task task4 = task3.ContinueWith((_) => { n = n - 2; Console.WriteLine("===> 4"); handle.Done = true; });
                    task4.Ignore();
                }), context);
            // ReSharper restore AccessToModifiedClosure
        ...
        private static OrleansTaskSchedulerAdvancedTests StartTestInSeparateAppDomain(out AppDomain outDomain)
        {
            AppDomainSetup setup = new AppDomainSetup();
            setup.ApplicationBase = System.Environment.CurrentDirectory;
            outDomain = AppDomain.CreateDomain("SchedulerAdvancedTestsDomain", null, setup);
            string assemblyName = System.Reflection.Assembly.GetAssembly(typeof(OrleansTaskSchedulerAdvancedTests)).FullName;
            string typeName = typeof(OrleansTaskSchedulerAdvancedTests).FullName;
            OrleansTaskSchedulerAdvancedTests test = (OrleansTaskSchedulerAdvancedTests)outDomain.CreateInstanceFromAndUnwrap("UnitTests.dll", typeName);
            return test;
        }
    }
}
