...
namespace UnitTests.SchedulerTests
{
    ...
    public class OrleansTaskSchedulerBasicTests
    {
        ...
        {
            SynchronizationContext.SetSynchronizationContext(null);
            TraceLogger.SetTraceLevelOverrides(new List<Tuple<string, Logger.Severity>>()); // Reset Log level overrides
        ...
        public void Async_Task_Start_OrleansTaskScheduler()
        {
            ...
            OrleansTaskScheduler scheduler = UnitTestBase.InitializeSchedulerForTesting(cntx);
        ...
        public void Async_Task_Start_ActivationTaskScheduler()
        {
            OrleansTaskSchedulerBasicTests.InitSchedulerLogging();
            UnitTestSchedulingContext cntx = new UnitTestSchedulingContext();
            OrleansTaskScheduler masterScheduler = UnitTestBase.InitializeSchedulerForTesting(cntx);
        ...
        public void Sched_SimpleFifoTest()
        {
            ...
            OrleansTaskScheduler orleansTaskScheduler = UnitTestBase.InitializeSchedulerForTesting(context);
        ...
        private void Sched_ExampleRunOnOrleansScheduler()
        {
            ...
            OrleansTaskScheduler scheduler = UnitTestBase.InitializeSchedulerForTesting(context);
            ...
            try
            {
                Task task1 = new Task((object x) =>
                {
                    ...
                    // if this code return Task, make sure to Wait for it.
        ...
        public void Sched_Task_NewTask_ContinueWith_Wrapped_OrleansTaskScheduler()
        {
            ...
            OrleansTaskScheduler scheduler = UnitTestBase.InitializeSchedulerForTesting(rootContext);
        ...
        internal static void InitSchedulerLogging()
        {
            ...
            var traceLevels = new[]
            {
                Tuple.Create("Scheduler", Logger.Severity.Verbose3),
                Tuple.Create("Scheduler.WorkerPoolThread", Logger.Severity.Verbose2),
            };
