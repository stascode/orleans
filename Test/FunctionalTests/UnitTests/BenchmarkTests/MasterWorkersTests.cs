...
namespace UnitTests
{
    ...
    public class MasterWorkersTests : UnitTestBase
    {
        ...
        private async Task StartGrains()
        {
            ...
            masterGrain = GrainClient.GrainFactory.GetGrain<IMasterGrain>(GetRandomGrainId());
            aggregatorGrain = GrainClient.GrainFactory.GetGrain<IAggregatorGrain>(GetRandomGrainId());
            workerGrains = new List<IWorkerGrain>();
            for (int i = 0; i < NumWorkers; i++)
            {
                IWorkerGrain worker = GrainClient.GrainFactory.GetGrain<IWorkerGrain>(GetRandomGrainId());
