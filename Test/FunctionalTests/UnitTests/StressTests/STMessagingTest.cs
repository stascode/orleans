...
namespace UnitTests.StressTests
{
    ...
    public class STMessagingTest : UnitTestBase
    {
        ...
        {
            IErrorGrainWithAsyncMethods nonPersistentGrain = GrainClient.GrainFactory.GetGrain<IErrorGrainWithAsyncMethods>(GetRandomGrainId());
        ...
        public async Task STMessagingTest_AlotOfGrains()
        {
            for (int iteration = 0; iteration < 20; iteration++)
            {
                ...
                {
                    long key = keys[i];
                    IStressSelfManagedGrain grain = GrainClient.GrainFactory.GetGrain<IStressSelfManagedGrain>(key);
                    grains.Add(grain);
        ...
        public async Task STMessagingTest_LargeMsg()
        {
            for (int iteration = 0; iteration < 20; iteration++)
            {
                ...
                {
                    long key = keys[i];
                    IStressSelfManagedGrain g1 = GrainClient.GrainFactory.GetGrain<IStressSelfManagedGrain>(key);
                    grains.Add(g1);
        ...
        public async Task STMessagingTest_LargeMsgBlob_WriteToAzureTable()
        {
            ...
            while (clock.Elapsed < testDuration)
            {
                for (int iteration = 1; iteration <= 32; iteration++)
                {
                    ...
                    {
                        long key = i + 1;
                        IAzureTableStorageAccessSMGrain grain = GrainClient.GrainFactory.GetGrain<IAzureTableStorageAccessSMGrain>(key);
                        Assert.AreEqual(key, grain.GetPrimaryKeyLong());
                        Assert.AreEqual(key.ToString(CultureInfo.InvariantCulture), await grain.GetLabel());
                        grains[i] = grain;
                    }
        ...
        public async Task STMessagingTest_LargeMsgBlob_WriteToAzureBlob()
        {
            ...
            while (clock.Elapsed < testDuration)
            {
                for (int iteration = 1; iteration <= 20; iteration++)
                {
                    ...
                    {
                        long key = i + 1;
                        IAzureBlobStorageAccessSMGrain grain = GrainClient.GrainFactory.GetGrain<IAzureBlobStorageAccessSMGrain>(key);
                        Assert.AreEqual(key, grain.GetPrimaryKeyLong());
                        Assert.AreEqual(key.ToString(CultureInfo.InvariantCulture), await grain.GetLabel());
                        grains[i] = grain;
                    }
