...
namespace UnitTests.Streaming.Reliability
{
    ...
    public class StreamReliabilityTests : UnitTestBase
    {
        ...
        public const string AZURE_QUEUE_STREAM_PROVIDER_NAME = StreamTestUtils.AZURE_QUEUE_STREAM_PROVIDER_NAME;
        ...
        public void TestCleanup()
        {
            ...
            {
                DeleteAllAzureQueues(_streamProviderName, DeploymentId, TestConstants.DataConnectionString, logger);
            }
#if DELETE_AFTER_TEST
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Functional"), TestCategory("Streaming"), TestCategory("Reliability")]
        ...
        private async Task Test_PubSub_MultiConsumerSameGrain(string testName, string streamProviderName)
        {
            ...
            await StreamTestUtils.CheckPubSubCounts(when, 1, 0, _streamId, _streamProviderName, UnitTestStreamNamespace.StreamReliabilityNamespace);

            logger.Info("AddConsumer x 2 : StreamId={0} Provider={1}", _streamId, _streamProviderName);
            await consumerGrain.AddConsumer(_streamId, _streamProviderName);
            when = "After first AddConsumer";
            await StreamTestUtils.CheckPubSubCounts(when, 1, 1, _streamId, _streamProviderName, UnitTestStreamNamespace.StreamReliabilityNamespace);

            await consumerGrain.AddConsumer(_streamId, _streamProviderName);
            when = "After second AddConsumer";
            await StreamTestUtils.CheckPubSubCounts(when, 1, 2, _streamId, _streamProviderName, UnitTestStreamNamespace.StreamReliabilityNamespace);
