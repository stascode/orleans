...
namespace UnitTests.StreamingTests
{
    ...
    public class StreamLimitTests : UnitTestBase
    {
        ...
        public async Task SMS_Limits_FindMax_Consumers()
        {
            ...
            string streamProviderName = StreamReliabilityTests.SMS_STREAM_PROVIDER_NAME;
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Functional"), TestCategory("Streaming"), TestCategory("Limits")]
            ...
            bool normalSubscribeCalls)
        {
            ...
            {
                // Producers
                var p1 = SetupProducers(streamId, streamProviderName, pipeline, numProducers);
            ...
            if (numConsumers > 0)
            {
                var c = SetupConsumers(streamId, streamProviderName, pipeline, numConsumers, normalSubscribeCalls);
                promises.AddRange(c);
            }
            ...
            {
                // Producers
                var p2 = SetupProducers(streamId, streamProviderName, pipeline, numProducers);
        ...
        private static IList<Task> SetupProducers(Guid streamId, string streamProviderName, AsyncPipeline pipeline, int numProducers)
        {
        ...
        private static IList<Task> SetupConsumers(Guid streamId, string streamProviderName, AsyncPipeline pipeline, int numConsumers, bool normalSubscribeCalls)
        {
            ...
            bool useFanOut = true)
        {
            ...
            await InitializeTopology(streamId, streamProviderName,
        ...
        private static async Task InitializeTopology(Guid streamId, string streamProviderName,
            ...
            bool useFanOut)
        {
            ...
            for (int loopCount = 0; loopCount < numConsumers; loopCount++)
            {
                ...
                Task promise = grain.BecomeConsumer(streamId, streamProviderName);
