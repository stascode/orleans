...
namespace UnitTests.StorageTests
{
    ...
    public class AzureQueueAdapterTests
    {
        ...
        [TestMethod, TestCategory("BVT"), TestCategory("Functional"), TestCategory("Halo"), TestCategory("Azure"), TestCategory("Streaming")]
        ...
        private async Task SendAndReceiveFromQueueAdapter(IQueueAdapterFactory adapterFactory, IProviderConfiguration config)
        {
            ...
            List<IQueueAdapterReceiver> receivers = mapper.GetAllQueues().Select(queueId => adapter.CreateReceiver(queueId)).ToList();
            List<IQueueCache> caches = mapper.GetAllQueues().Select(queueId => cache.CreateQueueCache(queueId)).ToList();
                ...
                Task task = Task.Factory.StartNew(() =>
                {
                    while (receivedBatches < NumBatches)
                    {
                        ...
                        foreach (AzureQueueBatchContainer message in messages.Cast<AzureQueueBatchContainer>())
                        {
                            ...
                            Console.WriteLine("Queue {0} received message on stream {1}", receiver.Id,
                                message.StreamGuid);
            ...
            foreach (KeyValuePair<QueueId, HashSet<Guid>> kvp in streamsPerQueue)
            {
                ...
                var qCache = caches.First(r => r.Id.Equals(kvp.Key));
