...
namespace UnitTests.Streaming
{
    public class SingleStreamTestRunner
    {
        ...
        public const string AQ_STREAM_PROVIDER_NAME = StreamTestUtils.AZURE_QUEUE_STREAM_PROVIDER_NAME;
        ...
        public async Task StreamTest_19_ConsumerImplicitlySubscribedToProducerClient()
        {
            ...
            const string consumerTypeName = "UnitTestGrains.Streaming_ImplicitlySubscribedConsumerGrain";
            ...
            Func<Task<bool>> waitUntilFunc =
                async () =>
                    0 < await UnitTestBase.GetActivationCount(consumerTypeName) && await CheckCounters(producer, consumer, assertAreEqual: false);
            await producer.ProduceSequentialSeries(ItemCount);
            await UnitTestBase.WaitUntilAsync(waitUntilFunc, _timeout);
        ...
        public async Task StreamTest_21_GenericConsumerImplicitlySubscribedToProducerGrain()
        {
            ...
            const string consumerTypeName = "UnitTestGrains.Streaming_ImplicitlySubscribedGenericConsumerGrain";
            Guid streamGuid = Guid.NewGuid();
            producer = await ProducerProxy.NewProducerGrainsAsync(streamGuid, streamProviderName, "TestNamespace1", logger);
            consumer = ConsumerProxy.NewConsumerGrainAsync_WithoutBecomeConsumer(streamGuid, logger, consumerTypeName);
        ...
        private async Task AssertProducerCount(int expectedCount, string providerName, Guid streamId)
        {
            ...
            {
                var actualCount = await StreamTestUtils.GetStreamPubSub().ProducerCount(streamId, providerName, UnitTestStreamNamespace.DefaultStreamNamespace);
