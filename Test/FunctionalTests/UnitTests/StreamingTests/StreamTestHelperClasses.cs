        ...
        public static Task<ConsumerProxy> NewConsumerGrainsAsync(Guid streamId, string streamProvider, TraceLogger logger, Guid[] grainIds = null, int grainCount = 1)
        {
            ...
            for (var i = 0; i < grainCount; ++i)
            {
                if (grainIds != null)
                {
                    ...
                    {
                        var gref = GrainClient.GrainFactory.GetGrain<IStreaming_ConsumerGrain>(grainIds[i], "UnitTestGrains.Streaming_ConsumerGrain");
                        grains[i] = gref;
                ...
                {
                    grains[i] = GrainClient.GrainFactory.GetGrain<IStreaming_ConsumerGrain>(Guid.NewGuid(), "UnitTestGrains.Streaming_ConsumerGrain");
                }
            }
        ...
        public static Task<ConsumerProxy> NewProducerConsumerGrainsAsync(Guid streamId, string streamProvider, TraceLogger logger, int[] grainIds, bool useReentrantGrain)
        {
            ...
            for (var i = 0; i < grainCount; ++i)
            {
                    ...
                    else
                    {
                        ...
                        {
                            grains[i] = GrainClient.GrainFactory.GetGrain<IStreaming_ProducerConsumerGrain>(grainIds[i], "UnitTestGrains.Streaming_ProducerConsumerGrain");
                        }
        ...
        public static Task<ProducerProxy> NewProducerGrainsAsync(Guid streamId, string streamProvider, string streamNamespace, TraceLogger logger, Guid[] grainIds = null, int grainCount = 1)
        {
            ...
            for (var i = 0; i < grainCount; ++i)
            {
                if (grainIds != null)
                {
                    ...
                    {
                        var gref = GrainClient.GrainFactory.GetGrain<IStreaming_ProducerGrain>(grainIds[i], "UnitTestGrains.Streaming_ProducerGrain");
                        grains[i] = gref;
                        dedup[grainIds[i]] = gref;
                    }
                }
                ...
                {
                    grains[i] = GrainClient.GrainFactory.GetGrain<IStreaming_ProducerGrain>(Guid.NewGuid(), "UnitTestGrains.Streaming_ProducerGrain");
                }
            }
