...
namespace UnitTests
{
    ...
    public class LoggerTest
    {
        ...
        public void SetUpDefaults()
        {
            ...
            TraceLogger.SetRuntimeLogLevel(Logger.Severity.Verbose);
            TraceLogger.SetAppLogLevel(Logger.Severity.Info);
            var overrides = new [] {
                new Tuple<string, Logger.Severity>("Runtime.One", Logger.Severity.Warning),
                new Tuple<string, Logger.Severity>("Grain.Two", Logger.Severity.Verbose3)
            };
        ...
        public void Logger_ConstructorTest()
        {
            ...
            Assert.AreEqual(Logger.Severity.Verbose, target.SeverityLevel,
            ...
            Assert.AreEqual(Logger.Severity.Warning, target.SeverityLevel,
            ...
            Assert.AreEqual(Logger.Severity.Verbose3, target.SeverityLevel,
                "Default severity level not calculated correctly for runtime logger with an override");
        }
        ...
        public void Logger_ConstructorTest1()
        {
            ...
            Assert.AreEqual(Logger.Severity.Info, target.SeverityLevel,
        ...
        public void Logger_SetRuntimeLogLevelTest()
        {
            ...
            Assert.AreEqual(Logger.Severity.Verbose, target.SeverityLevel,
                "Default severity level not calculated correctly for runtime logger with no overrides");
            TraceLogger.SetRuntimeLogLevel(Logger.Severity.Warning);
            Assert.AreEqual(Logger.Severity.Warning, target.SeverityLevel,
        ...
        public void Logger_SeverityLevelTest()
        {
            ...
            Assert.AreEqual(Logger.Severity.Verbose, target.SeverityLevel,
                "Default severity level not calculated correctly for runtime logger with no overrides");
            target.SetSeverityLevel(Logger.Severity.Verbose2);
            Assert.AreEqual(Logger.Severity.Verbose2, target.SeverityLevel,
        ...
        public void Logger_AddRemoveOverride()
        {
            ...
            TraceLogger.AddTraceLevelOverride(fullName, Logger.Severity.Warning);
            Assert.AreEqual(Logger.Severity.Warning, logger.SeverityLevel, "Logger level not reset after override added");
        ...
        private void RunLogWriterPerfTest(string testName, int n, int logCode, TimeSpan target, ILogConsumer log)
        {
            ...
            for (int i = 0; i < n; i++)
            {
                log.Log(Logger.Severity.Info, TraceLogger.LoggerType.Runtime, testName, "msg " + i, null, null, logCode);
            }
            stopwatch.Stop();
    ...
    class TestLogConsumer : ILogConsumer
    {
        ...
        public void Log(Logger.Severity severity, TraceLogger.LoggerType loggerType, string caller, string message, System.Net.IPEndPoint myIPEndPoint, Exception exception, int eventCode = 0)
        {
            lock (this)
            {
