...
namespace UnitTests
{
    ...
    [DeploymentItem("UnitTestGrainInterfaces.dll")]
    [DeploymentItem("UnitTestGrains.dll")]
    public class UnitTestBase
    {
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception(
                  string.Format("Exception during test initialization: {0}",
                      TraceLogger.PrintException(ex.GetBaseException())));
            }
        }
        ...
        {
            var grainClient = new OutsideRuntimeClient(ClientConfig, new GrainFactory(), true);
        ...
        static SiloHandle StartOrleansRuntime(SiloHandle retValue, Silo.SiloType type, Options options, AppDomain shared = null)
        {
            ...
            if (!options.EnforceMinimumRequirementForAgeLimit)
            {
        ...
        protected static void EmptyMembershipTable()
        {
            try
            {
                ...
                IMembershipTable table=null;

                if (config.Globals.LivenessType.Equals(GlobalConfiguration.LivenessProviderType.AzureTable))
                {
                    table = new AzureBasedMembershipTable();
                }
                else if (config.Globals.LivenessType.Equals(GlobalConfiguration.LivenessProviderType.SqlServer))
                {
                    table = new SqlMembershipTable();
                }

                if (table != null)
                {
                    table.InitializeMembershipTable(config.Globals, true, TraceLogger.GetLogger(table.GetType().Name))
                        .WithTimeout(AzureTableDefaultPolicies.TableOperationTimeout).Wait();
  ...
  <Override Node=""Node3"">
    <Networking Port=""11114"" />
        ...
        private static string ToXmlString(ApplicationConfiguration appConfig)
        {
            ...
            foreach (GrainTypeConfiguration classConfig in appConfig.ClassSpecific)
            {
                ...
                {
                    result.AppendFormat("                <GrainType Type=\"{0}\">", classConfig.FullTypeName);
    ...
    public class Options
    {
        public Options()
        {
            ...
            EnforceMinimumRequirementForAgeLimit = true;
        }
        ...
        public Options Copy()
        {
            return new Options
            {
                ...
                EnforceMinimumRequirementForAgeLimit = EnforceMinimumRequirementForAgeLimit
            };
        }
        ...
        public bool EnforceMinimumRequirementForAgeLimit { get; set; }
    }
